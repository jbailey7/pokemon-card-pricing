...
def pairwise_l2_distance(embeddings: torch.Tensor) -> torch.Tensor:
    ...
    dot = dot.clamp(-1, 1)
    sq_dist = (2.0 - 2.0 * dot).clamp(min=0)
...
def batch_hard_triplet_loss(
    ...
    # Skip anchors with no positives in the batch (can happen at PKSampler edges)
...
def evaluate(
    ...
    # Embed the val set twice: once clean (gallery) and once augmented (query),
    # then measure how often the augmented version retrieves its clean counterpart.
    model.eval()
    pin = device.type == "cuda"

    def embed(transform) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    gallery_emb, gallery_labels = embed(build_val_transform())
    query_emb, query_labels = embed(build_train_transform())

    n = query_emb.size(0)
    dot = (query_emb @ gallery_emb.t()).clamp(-1, 1)
    dist = (2.0 - 2.0 * dot).clamp(min=0).add(1e-8).sqrt()
